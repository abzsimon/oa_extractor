      ...
      <div className="grid grid-cols-2 gap-x-4 gap-y-2 mt-2 text-sm">
        ...
        <div className="flex items-center">
          ...
          <span className={`px-1.5 py-0.5 rounded-full text-xs font-medium ${
            typeof article.oa_status === 'string' && article.oa_status.toLowerCase() === 'open' 
              ? 'bg-green-100 text-green-800' 
              : 'bg-amber-100 text-amber-800'
          }`}>
            {article.oa_status || "Closed"}
          </span>
        </div>
      </div>
          ...
          <div className="flex items-center mb-1">
            ...
            <div className="flex flex-wrap gap-1">
              {article.domains.map((domain, index) => (
                <span key={index} className="bg-blue-50 text-blue-700 text-xs font-medium px-1.5 py-0.5 rounded">
                  {domain}
                </span>
              ))}
            </div>
          ...
          <div className="flex items-center mb-1">
            ...
            <div className="flex flex-wrap gap-1">
              {article.fields.map((field, index) => (
                <span key={index} className="bg-purple-50 text-purple-700 text-xs font-medium px-1.5 py-0.5 rounded">
                  {field}
                </span>
              ))}
            </div>
          ...
          <div className="flex items-center">
            ...
            <div className="flex flex-wrap gap-1">
              {article.subfields.map((subfield, index) => (
                <span key={index} className="bg-teal-50 text-teal-700 text-xs font-medium px-1.5 py-0.5 rounded">
                  {subfield}
                </span>
              ))}
            </div>
