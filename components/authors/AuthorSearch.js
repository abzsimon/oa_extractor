...
export default function OpenAlexAuthorDisambiguation() {
  ...
  const handleSelectAuthor = async (author) => {
    ...
    try {
      // 📥 1. Récupère les infos de base de l'auteur
      const res = await fetch(`https://api.openalex.org/authors/${oa_id}`);
      const data = await res.json();

      // 📘 2. Récupère la liste complète des IDs de travaux
      const worksRes = await fetch(`${data.works_api_url}&select=id&per_page=200`);
      const worksData = await worksRes.json();
      const overall_works = (worksData.results || []).map(w =>
        w.id.replace('https://openalex.org/', '')
      );

      // 📚 3. Récupère les types de documents
      const typesRes = await fetch(`${data.works_api_url}&group_by=type`);
      const typesData = await typesRes.json();
      const doctypes = (typesData.group_by || []).map(d => ({
        name: d.key_display_name,
        quantity: d.count
      }));

      // 🌳 4. Construit l'arbre thématique
      const topic_tree = buildTopicTree(data.topics || []);

      // 🔝 5. Dérive les top domaines et champs
      const top_two_domains = getTopTwoDomains(topic_tree);
      const top_five_fields = getTopFiveFields(topic_tree);

      // 🏷 6. Top 5 topics simples
      const top_five_topics = (data.topics || [])
        .slice(0, 5)
        .map(t => t.display_name);

      // 🧩 7. Structure complète de l'auteur
      ...
      <div className="flex flex-col gap-6">
        ...
        <div className="w-full flex items-center justify-between bg-gray-50 p-4 rounded-lg shadow-sm">
          <div className="flex items-center gap-3">
            ...
            />
            <h1 className="text-xl font-bold text-gray-800">OpenAlex Author Search</h1>
          </div>
        ...
        <div className="w-full text-center">
          {disambiguationQuery.length > 0 && disambiguationQuery.length < 10 && (
            <p className="text-amber-600">Please enter at least 10 characters to search</p>
          )}
          {isLoading && <p className="text-blue-600">Searching for authors...</p>}
          {!isLoading && authors.length === 0 && disambiguationQuery.length >= 10 && (
            <p className="text-gray-600">No authors found matching your query.</p>
          )}
        </div>
                  ...
                  >
                    {selectedAuthor?.oa_id === author.oa_id && (
                      <div className="absolute inset-0 bg-white bg-opacity-70 flex items-center justify-center rounded-lg">
                        <p className="text-blue-600 font-medium animate-pulse">Loading author data...</p>
                      </div>
                    )}
                    ...
                    <div className="flex justify-between items-start mb-2">
                      ...
                      {isSelected && (
                        <span className="bg-red-100 text-red-800 text-xs px-2 py-1 rounded-full">Selected</span>
                      )}
                    </div>
                      ...
                      <div className="mb-2">
                        {author.oa_id && (
                          <div className="border border-gray-300 rounded px-2 py-1 bg-gray-50 mb-1 flex items-center">
                            <span className="font-medium text-gray-600 mr-1 text-xs">OpenAlex ID:</span>
                            <span className="text-blue-600 text-xs truncate">{author.oa_id}</span>
                          </div>
                        )}
                        ...
                        {author.orcid_id ? (
                          ...
                          >
                            <span className="text-green-700 text-xs mr-1">ORCID:</span>
                            <span className="text-green-600 text-xs truncate">{author.orcid_id}</span>
                          </a>
                        ) : (
